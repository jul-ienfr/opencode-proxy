...
[Phase 2 refonte] Domicile CANONIQUE (déplacement pur depuis
``traffic_capture.py`` — contenu identique à l'octet près, seul cet en-tête
change). ``traffic_capture.py`` reste un shim de re-export (façade gelée
ADR-006) jusqu'à la Phase 9 ; le nouveau code importe ``observability.capture``.
