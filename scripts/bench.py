...
async def bench_health(url, conc, total):
    async with httpx.AsyncClient(timeout=10) as c:
        ...
        print(f"p50 {statistics.median(lat):.1f}ms p95 {sorted(lat)[int(len(lat)*0.95)]:.1f}ms p99 {sorted(lat)[int(len(lat)*0.99)]:.1f}ms")

async def bench_stream(url):
    body = {"model":"mimo-v2.5","messages":[{"role":"user","content":"ping"}],"stream":True,"max_tokens":32}
    t0 = time.perf_counter()
