...
async def test_forced_rotation(base: str) -> None:
    ...
    check("total_switches incremented",
          after_switches > before_switches, f"{before_switches} → {after_switches}")
    ...
    if rot_on:
        check("identity advanced with rotation",
              after_identity is not None and after_identity != before_identity,
              f"{before_identity} → {after_identity}")
    else:
        check("identity stable (rotation off)",
              after_identity == before_identity, f"{before_identity} → {after_identity}")
    check("circuit_breaker well-formed",
          cb.get("state") in ("closed", "open", "half_open"), str(cb.get("state")))
    ...
    async with httpx.AsyncClient(base_url=base, timeout=120) as client:
        ...
        try:
            ...
            try:
                ...
                hc_ok, hc_body = False, {}
            check("outage: health-check clean JSON (ok=false)",
            ...
            if r.status_code == 200:
                try:
                    ...
                    ok = False
                check("outage: fail-open answer (direct or self-healed)",
                      ok, r.text[:120] if not ok else "")
            else:
                try:
                    b = r.json()
                    ...
                    ok = False
                check("outage: structured JSON error",
...
async def test_quota_rotation(base: str, token: str) -> None:
    ...
    async with httpx.AsyncClient(base_url=base, timeout=180) as client:
        ...
        if original_quota is None:
            ...
            return
        try:
            r = await client.post("/api/vpn-config",
                                  json={"quota_per_ip": 12}, headers=headers)
            body = r.json() if r.status_code == 200 else {}
            if r.status_code != 200 or not body.get("ok"):
                check("quota rotation", False,
            ...
            for i in range(2):
                ...
                if resp.status_code != 200:
                    check("quota rotation", False,
                          f"free request {i + 1} HTTP {resp.status_code}")
                    return
            ...
            rr = await client.post("/api/vpn-config",
                                   json={"quota_per_ip": original_quota},
                                   headers=headers)
...
async def test_update_contract(base: str) -> None:
    ...
    async with httpx.AsyncClient(base_url=base, timeout=120) as client:
        ...
        try:
            ...
            ok = False
        check("update endpoint clean JSON", r.status_code == 200 and ok,
...
async def main() -> None:
    ...
    parser.add_argument("--token", default="", help="dashboard token (else DASHBOARD_TOKEN env/.env)")
    parser.add_argument("--quota-trigger", action="store_true",
                        help="run test_quota_rotation (temporarily lowers quota_per_ip)")
    args = parser.parse_args()
    base = f"http://127.0.0.1:{args.port}"

    print(f"target: {base}  container: {CONTAINER}")
    tests: list = [test_smoke_non_stream, test_smoke_stream,
