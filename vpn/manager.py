...
[Phase 6 refonte — chantier 3] Domicile CANONIQUE (déplacement pur depuis
``vpn_manager.py`` — contenu identique à l'octet près, sauf les 3
ajustements marqués [Phase 6] ci-dessous, tous à comportement identique) :
``vpn_manager.py`` reste un shim de re-export jusqu'à la Phase 9 (et 2
versions au-delà, plan §10) ; le nouveau code importe ``vpn.manager``.
La découpe fine stations/compose/watchdog/geo reste en follow-up.
"""
...
async def ensure_docker_running(timeout: int = 60) -> bool:
    ...
    if not exe:
        ...
        try:
            ...
            logging.getLogger("vpn_manager").warning("[docker] failed to launch Desktop %s: %s", exe, e)
            return False
    ...
    for _ in range(timeout // 2):
        ...
        try:
            ...
            if r.returncode == 0:
                logging.getLogger("vpn_manager").info("[docker] daemon reachable after Desktop launch")
                return True
        except Exception:
            pass
    logging.getLogger("vpn_manager").warning("[docker] daemon still not reachable after %ds", timeout)
    return False
...
# [Phase 6] nom de logger FIGÉ à "vpn_manager" (5 sites getLogger(__name__)
# → littéral) : opencode attache ce logger à debug.log (attach_module_logger,
# filet G7 anti-silence) — __name__ vaudrait ici "vpn.manager" et les
# enregistrements ne seraient plus routés.
...
# Project root
# [Phase 6] ancré RACINE repo (ce fichier vit dans vpn/ — un niveau plus bas
# que l'original) : SANS ce parent supplémentaire, logs/vpn_state*.json,
# vpn_configs/credentials.txt et le lock file partiraient dans vpn/.
...
class VPNManager:
    ...
    def __init__(self, cfg: dict, station: int = 1, shared=None):
        ...
        if isinstance(_ovr, dict) and _ovr:
            cfg = {**cfg, **_ovr}
            logging.getLogger("vpn_manager").info(
...
def _default_docker_runner(args, timeout=30, env=None):
    """Runner docker par défaut de reconcile_orphan_containers (boot).

    [Phase 6] ``_docker_cli`` est résolu via le shim ``vpn_manager``
    (namespace que les tests patchent :
    ``monkeypatch.setattr(vm, "_docker_cli", fake)``). Une référence
    directe au global canonique rendrait le patch invisible (le shim ne
    fait que ré-exporter — le rebind n'y est visible que via SON
    attribut). En prod le shim ré-exporte cette même fonction :
    comportement identique. Fallback canonique si le shim est indisponible.
    """
    try:
        import vpn_manager as _shim

        _cli = getattr(_shim, "_docker_cli", None)
    except Exception:
        _cli = None
    return asyncio.to_thread(_cli if callable(_cli) else _docker_cli, args, timeout, env)
...
async def reconcile_orphan_containers(managers: list, runner=None) -> list[str]:
    ...
    if not managers:
        return []
    run = runner or _default_docker_runner
