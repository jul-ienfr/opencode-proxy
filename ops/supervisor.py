...
[Phase 8 refonte] Domicile CANONIQUE (déplacement pur depuis
``station_supervisor.py`` — contenu identique à l'octet près, seul cet
en-tête change ; zéro ``global`` : état en instances/constantes, shim sans
divergence). ``station_supervisor.py`` reste un shim de re-export jusqu'à
la Phase 9 ; le nouveau code importe ``ops.supervisor``.
"""
