...
class _StubMgr:
    ...
    def __init__(self, config, station, shared=None):
        ...
        self._compose_service = ("vpn-gluetun" if station <= 1
                                 else f"vpn-gluetun-{station}")
        self._docker_container = ("opencode-vpn" if station <= 1
                                  else f"opencode-vpn-{station}")
...
async def test_upscale_3_syncs_env_stack_before_any_start(tmp_path, monkeypatch):
    ...
    _seed_env(tmp_path, VPN_TYPE_STATION1="wireguard",
              VPN_TYPE_STATION2="wireguard", SOME_SECRET="abc123")
...
async def test_downscale_3_to_2_stops_container_no_stack_call(tmp_path, monkeypatch):
    ...
    stubs = [_StubMgr(_cfg(tmp_path), station=1),
