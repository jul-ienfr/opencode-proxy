...
class StationSupervisor:
    ...
    # ── latence / rotation (API gelée Lot 1, remplie Lot 3) ───────────

    def tracker_for(self, ip: str) -> IpLatencyTracker:
        tr = self.ip_latency.get(ip)
        if tr is None:
            tr = IpLatencyTracker(station=self.station, ip=ip)
            self.ip_latency[ip] = tr
            # borne mémoire : 1 ring/IP × ~30 IPs récentes max par station
            while len(self.ip_latency) > 30:
                oldest = next(iter(self.ip_latency))
                self.ip_latency.pop(oldest, None)
        return tr
    ...
    def should_soft_rotate(self, model: str, thresholds: dict[str, float]) -> bool:
        """Gelé Lot 1 → délègue au tracker (False constant). Lot 3 branchera
        les seuils per-model + global_degraded + anti-flapping."""
        threshold_for = float(thresholds.get(model, thresholds.get("default", 8000)))
        p95_threshold = float(thresholds.get("p95", 9000))
        active_ip = str(getattr(self.manager, "current_ip", "") or "")
        tr = self.tracker_for(active_ip)
        return tr.should_soft_rotate(threshold_for, p95_threshold)
